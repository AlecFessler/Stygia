// libc_smoke — smallest possible consumer of libz/libc/libc.a.
//
// Spawned by the parent (root_service) with COM1 in cap-table slot 3.
// Calls libc's printf("...") to verify the full chain works:
//   printf → vfprintf → fputc → fwrite → write → zag_write_console → COM1
//
// If we see the banner on serial, the libc.a static archive +
// minimal-runtime pattern is proven sound. That's the green light
// for Phase 4c.4 (full LLVM cross-build).
//
// All zag_* extern definitions live below; libc.a refers to them
// by linker name. Most are stubs since the smoke test only exercises
// the write path; the runtime is sized so the linker sees defs for
// every extern libc.a pulls in.

// ── extern libc fns we test ──────────────────────────────────────

extern fn printf(fmt: [*:0]const u8, ...) callconv(.c) c_int;
extern fn exit(status: c_int) callconv(.c) noreturn;

// ── Cap-table layout / syscall plumbing (mirrors zig_hello2) ─────
...
fn initCom1(cap_table_base: u64) void {
...
// ── zag_* runtime defs (referenced by libc.a) ────────────────────
...
// ── fs / mmap stubs (smoke test doesn't exercise these) ───────────
...
// Anon mmap — give back zero-fill demand-paged VMARs. For the smoke
// test malloc may be reached transitively; provide a working impl.
...
export fn zag_munmap(addr: u64, pages: usize) callconv(.c) i32 {
    _ = .{ addr, pages };
    return 0; // VMAR cleanup happens at process exit; leak for now.
}

// ── Entry point ──────────────────────────────────────────────────

export fn _start(cap_table_base: u64) callconv(.c) noreturn {
    initCom1(cap_table_base);

    // Direct COM1 print to confirm we got control.
    if (com1_sink) |p| {
        const banner = "[libc_smoke] _start running\n";
        var i: usize = 0;
        while (i < banner.len) : (i += 1) p[0] = banner[i];
    }

    // The actual smoke test: libc's printf via the full chain.
    _ = printf("[libc_smoke] hello via libc printf — %s + %d\n", @as([*:0]const u8, "string"), @as(c_int, 42));

    if (com1_sink) |p| {
        const banner = "[libc_smoke] returned from printf\n";
        var i: usize = 0;
        while (i < banner.len) : (i += 1) p[0] = banner[i];
    }
