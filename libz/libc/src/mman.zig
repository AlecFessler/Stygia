...
//
// Anonymous mmap routes through zag_mmap_anon (the consumer ELF
...
export fn munmap(addr: ?*anyopaque, length: usize) callconv(.c) c_int {
    ...
    const pages = (length + PAGE - 1) / PAGE;
    return zag_munmap(@intFromPtr(addr.?), pages);
}
...
// shm_open / shm_unlink — Zag has no POSIX shm. Stub ENOSYS.
