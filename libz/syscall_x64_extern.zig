...
// stack at [rsp + 8 .. rsp + 480] (vregs 14..73). Uses the same static
// `vm_exit_reply_buf` as replyTransferVmExitAsm — Zig writes the
// syscall word at index 0 and the sub-code at index 57 (vreg 70), then
// the asm points rsp at the buffer for the syscall.
pub fn replyVmExitAsm(word: u64, subcode: u64) Regs {
    @memset(vm_exit_reply_buf[0..115], 0);
    vm_exit_reply_buf[0] = word;
    vm_exit_reply_buf[57] = subcode;
    ...
    var ov13: u64 = undefined;
    asm volatile (
        \\ movq %%rsp, vm_exit_reply_saved_rsp(%%rip)
        \\ leaq vm_exit_reply_buf(%%rip), %%rsp
        \\ movq (%%rsp), %%rcx
        \\ syscall
        \\ movq vm_exit_reply_saved_rsp(%%rip), %%rsp
        : [v1] "={rax}" (ov1),
          [v2] "={rbx}" (ov2),
          [v3] "={rdx}" (ov3),
          [v4] "={rbp}" (ov4),
...
// handles in vregs [128-N..127]. Uses a static `vm_exit_reply_buf`
// pre-populated by Zig before the syscall, then points rsp at the
// buffer for the syscall (mirrors hyprvOS's vm_exit_buf pattern). This
// avoids the SysV-AMD64 register-pressure trap on the inline asm —
// only the syscall word and the rsp swap need register operands.
export var vm_exit_reply_buf: [128]u64 align(16) = .{0} ** 128;
export var vm_exit_reply_saved_rsp: u64 align(8) = 0;

pub fn replyTransferVmExitAsm(word: u64, attachments_ptr: [*]const u64, n: u64, subcode: u64) Regs {
    // Buffer index N → vreg N+13. Vreg 0 = buffer index -13 mapped to
    // index 0 below the syscall window (impossible). Instead the syscall
    // word lives at index 0; vreg 14 at index 1; vreg 70 at index 57;
    // vreg 127 at index 114. Total used: 115 u64s.
    @memset(vm_exit_reply_buf[0..115], 0);
    vm_exit_reply_buf[0] = word; // vreg 0 (syscall word)
    vm_exit_reply_buf[57] = subcode; // vreg 70 (exit subcode)
    // Pair entries at vregs [128-N..127] → buffer indices [115-N..114].
    const k_first: usize = 115 - n;
    var i: usize = 0;
    while (i < n) : (i += 1) {
        vm_exit_reply_buf[k_first + i] = attachments_ptr[i];
    }
