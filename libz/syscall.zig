...
// Public ABI surface — every name here becomes a global dynamic symbol
// in libz.elf, resolvable by the userspace rtld in libz_loader. The
// matching extern-fn declarations live in libz/api.zig; apps that
// dynamically link against libz import api.zig and the linker emits
// R_X86_64_GLOB_DAT / R_X86_64_JUMP_SLOT relocations against these
// names. Adding a wrapper above without listing it here keeps it
// static-only — visible to consumers that import this file directly
// (e.g. the test runner, root_service), invisible to dynamic consumers.
comptime {
    // Cap-table-wide ops
