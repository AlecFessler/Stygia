...
//     - one VMAR per cur_rwx-before-remap variant + one new_cur_rwx
...
//      succeed; a regular VMAR (caps.dma=0, caps.mmio=0) starting in
//      `map = 0` per §[var].
//   3. map_pf(var, &.{ 0, pf }) — installs the page_frame at offset
//      0, transitioning `map` to 1 (§[map_pf] test 11). Effective
//      permissions on VMAR.base[0..4096] are now (r|w) ∩ (r|w) = r|w.
//   4. Pre-remap probe: write 0xA5 to VMAR.base[0]; read back. Both
...
//      under test), effective permissions on VMAR.base[0..4096] become
...
//
// Assertions
//   1: setup failed — create_page_frame, create_vmar, map_pf, or the
...
//   2: pre-remap byte didn't round-trip — VMAR.base[0] read back a
...
//   variants once §[create_vmar] gating is set up), pair each with a
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 2: regular VMAR with caps={r, w} and cur_rwx = r|w. Per
    ...
    // 11, `map` transitions 0 -> 1, so the VMAR is now eligible for
    // remap (§[remap] test 02 rejects map=0). Effective permissions
    // on VMAR.base[0..4096] become (r|w) ∩ (r|w) = r|w.
    ...
    // the read at VMAR.base[0] succeed and the byte round-trips. The
