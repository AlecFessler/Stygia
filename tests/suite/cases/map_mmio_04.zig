...
//  mappings are atomic; the VMAR must be unmapped)."
//
// DEGRADED SMOKE VARIANT
//   The strict test 04 path requires landing a non-zero `map` on a
//   VMAR whose `caps.mmio` bit is set, then issuing map_mmio so the
...
//     with E_PERM on any VMAR whose `caps.mmio` is set. So map_pf
//     cannot drive an MMIO VMAR's `map` to 1 — it errors out before
//     touching field1. Inverting the construction (regular VMAR with
//     caps.mmio = 0, drive `map` to 1 via map_pf, then map_mmio)
//     hits §[map_mmio] test 03 first: a non-mmio VMAR yields E_PERM
...
//     access. An MMIO VMAR's faulting semantics are not a demand
//     allocation — they are gated by §[map_mmio] entirely. So an
//     MMIO VMAR cannot land in `map = 3` by any path.
//
//   With no construction landing `map ∈ {1, 2, 3}` on an MMIO VMAR
...
//   VMAR (caps = {r, w}, mmio = 0) plus a freshly-minted page_frame
//   admit a successful map_pf, transitioning the VMAR's `map` to 1.
//   That construction is the *closest reachable* approximation of
//   the test 04 pre-state — it lands `map = 1` on the same VMAR
//   shape used by every other map_* test — but the follow-up
//   map_mmio call against this VMAR rejects with E_PERM (test 03)
...
//       blocks reaching test 04 from a non-mmio VMAR.
//     - test 03 (caps.mmio not set) — fires here because the VMAR
...
//   3. mapPf(vmar_handle, &.{ 0, pf }) — must succeed (transitions
//      `map` to 1 per §[map_pf] test 11).
//   4. mapMmio(vmar_handle, 4095) — issues the call so the test
...
pub fn main(cap_table_base: u64) void {
    ...
    // VMAR's r/w so cur_rwx ∩ pf.caps stays r|w on success.
    ...
    // the test 04 pre-state is to drive this VMAR's `map` to 1 via
    // map_pf — the spec rejects map_mmio against an MMIO VMAR with
    // map = 1 via test 04, but the only way to land map = 1 on a
    // VMAR is map_pf, which itself rejects MMIO VARs via §[map_pf]
    // test 03. So this prelude lands map = 1 on a non-mmio VMAR;
    ...
    // VMAR is now in the closest-reachable approximation of the
    // test 04 pre-state — `map != 0` on a VMAR — but caps.mmio = 0,
    ...
    _ = syscall.mapMmio(vmar_handle, 4095);

    // No spec assertion is being checked — the test 04 rejection
    // requires an MMIO VMAR with `map != 0`, which is unreachable
