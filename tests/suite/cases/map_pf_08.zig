...
//     - test 03 (caps.mmio = 1)       — VMAR caps = {r,w}, mmio = 0.
//     - test 04 (N == 0)              — N = 2.
//     - test 05 (offset misaligned)   — both offsets = 0, aligned to
//                                       any sz.
//     - test 06 (pf.sz < VMAR.sz)      — both share sz = 0 (4 KiB).
//     - test 07 (range exceeds VMAR)   — VMAR pages = 2 (8 KiB) and each
//                                       pf pages = 2 (8 KiB). A pair at
//                                       offset 0 covers exactly [0,
//                                       8 KiB), which fits.
//     - test 09 (overlaps existing)   — fresh VMAR has map = 0 with no
//                                       prior installations; only the
//                                       in-call overlap can fire.
//     - test 10 (map ∈ {2,3})         — fresh VMAR has map = 0.
//
//   With VMAR.size = 8 KiB and each pf covering 8 KiB, two pairs both
...
//      must succeed.
//   2. createVmar(caps={r,w}, props={sz=0, cur_rwx=0b011}, pages=2)
//      — must succeed.
...
//   2: a setup syscall (createPageFrame or createVmar) returned an
...
pub fn main(cap_table_base: u64) void {
    ...
    // therefore overlap.
    const result = syscall.mapPf(vmar_handle, &.{ 0, pf_a, 0, pf_b });
