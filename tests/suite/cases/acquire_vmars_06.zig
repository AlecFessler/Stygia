...
//   carries the domain's `vmar_outer_ceiling` in field1 bits 8-15;
//   the self-IDC at slot 2 carries `vmar_cap_ceiling` in its field0
//   bits 16-23.
//
//   We mint a `map=1` VMAR (regular VMAR with caps.r=true plus a single
//   `map_pf` call to install one page) so the target domain has at
//   least one acquire-able VMAR (per test 07: only `map=1` and `map=3`
//   VARs are returned). Then we call `acquire_vmars(SLOT_SELF_IDC)`.
//
//   The post-condition: every returned handle's caps field equals
//   `vmar_outer_ceiling ∩ vmar_cap_ceiling`. We read both values out of
//   the read-only cap table mapping (§[capabilities]) so we don't have
//   to encode a static value the runner is free to change.
//
//   SPEC AMBIGUITY: spec does not pin the initial `vmar_cap_ceiling` /
...
//   while acquire_vmars is in flight. For a self-IDC call the calling EC
...
pub fn main(cap_table_base: u64) void {
    // 1. Mint a page frame to back our acquire-able VMAR.
    ...
    // 2. Mint a regular VMAR (mmio=0, dma=0). caps.r|w with cur_rwx=r|w
    //    keeps create_vmar off the E_INVAL paths (cur_rwx must be a
    //    subset of caps r/w/x, sz=0 keeps mmio/dma constraints inert).
    const vmar_caps = caps.VmarCap{ .r = true, .w = true };
    // §[create_vmar] props: cur_rwx in bits 0-2, sz bits 3-4, cch bits 5-6.
    ...
    // Read the vmar_outer_ceiling out of the self-handle's field1
    // (bits 8-15) and the self-IDC's vmar_cap_ceiling out of its
    ...
    const vmar_outer_ceiling: u8 = @truncate((self_cap.field1 >> 8) & 0xFF);

    const self_idc_cap = caps.readCap(cap_table_base, caps.SLOT_SELF_IDC);
    const vmar_cap_ceiling: u8 = @truncate((self_idc_cap.field0 >> 16) & 0xFF);

    const expected_caps: u16 = @as(u16, vmar_outer_ceiling) & @as(u16, vmar_cap_ceiling);

    // 4. Acquire VARs through the self-IDC.
    const got = syscall.acquireVmars(caps.SLOT_SELF_IDC);
    ...
    // §[acquire_vmars] syscall word bits 12-19: count.
    ...
    while (i < inspect) {
        ...
        // The slot must reference a VMAR (handle type 4 per
