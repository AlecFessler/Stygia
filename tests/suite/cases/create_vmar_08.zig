// Spec §[create_vmar] create_vmar — test 08.
//
...
//   props.sz on an MMIO VMAR must produce E_INVAL.
//
...
//       runner/primary.zig.
//     - test 02 (E_PERM caps.r/w/x not subset of vmar_inner_ceiling):
//       runner's vmar_inner_ceiling = 0x01FF includes r|w|x|mmio,
...
//       vmar_inner_ceiling has mmio set, so this gate passes.
...
//       reserved bits are 0 via VmarCap defaults; props reserved
//       bits 7-63 are 0 since we only fill cur_rwx, sz, cch.
//
// Action
//   create_vmar(caps={r,w,mmio}, props={cur_rwx=0b011, sz=1, cch=0},
