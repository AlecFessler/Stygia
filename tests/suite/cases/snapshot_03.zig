...
//   there is to mint a single regular VMAR handle via `create_vmar` with
...
//
//   §[var] specifies the VmarCap layout: bits 9-10 hold restart_policy.
//   `caps.VmarCap{ .r = true, .w = true }` leaves restart_policy at its
...
pub fn main(cap_table_base: u64) void {
    ...
    const result = syscall.snapshot(vmar_handle, vmar_handle);
