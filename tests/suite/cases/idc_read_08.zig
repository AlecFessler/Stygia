...
//
// Strategy
//   `idc_read` is a pure read of VMAR contents — it never mutates the
//   VMAR's `map`, `device`, `cur_rwx`, or any other field1 state. The
...
//
//   Leg A (call against a fresh VMAR — refresh of unchanged state)
...
//     fits within the VMAR's 4 KiB size, offset is 8-byte aligned, the
//     handle has the `r` cap, count is in 1..125, no reserved bits
//     are set, and [1] is a valid VMAR — so none of §[idc_read] tests
...
//     kernel-side VMAR state is unchanged, so a refresh per test 08
//     leaves field1 bit-identical to the pre-call snapshot.
//
//   Leg B (call against a pf-mapped VMAR — refresh of changed state)
...
//     against the same VMAR — none of §[idc_read] tests 01-06 fire
...
//      cannot mutate kernel-side VMAR state (idc_read is a pure read).
...
//   8. readCap(cap_table_base, vmar_handle) — field1's `map` must be 1,
//      reflecting the kernel's authoritative post-map_pf state still
//      mirrored after idc_read's refresh.
//
// Assertions
//   1: a setup syscall (createVmar, createPageFrame, mapPf, or the
...
//      the VMAR left the slot out of sync with the kernel's
...
pub fn main(cap_table_base: u64) void {
    ...
    // §[var] line 877 it starts at `map = 0`. The VMAR has the `r` cap
    ...
    // no bound device.
    const cap_pre_a = caps.readCap(cap_table_base, vmar_handle);
    if (cap_pre_a.handleType() != caps.HandleType.virtual_memory_address_region) {
        testing.fail(1);
        return;
    }
    const field1_pre_a = cap_pre_a.field1;

    // Step 3: idc_read against the fresh VMAR. count = 1, offset = 0
    // (8-byte aligned, fits within the VMAR's 4 KiB size); the [1]
    // handle is a valid VMAR with the `r` cap and no reserved bits set
    ...
    // is a pure read of VMAR contents and cannot mutate kernel-side
    // VMAR state, so the authoritative state remains exactly what step
    // 2 snapshotted.
    _ = syscall.idcRead(vmar_handle, 0, 1);

    // Step 4: leg-A refresh — the kernel's authoritative VMAR state is
    // unchanged from step 2, so a refresh of field0/field1 must leave
    // the slot bit-identical to the pre-call snapshot.
    const cap_post_a = caps.readCap(cap_table_base, vmar_handle);
    ...
    // branch.
    const ok_call = syscall.idcRead(vmar_handle, 0, 1);
    if (ok_call.v1 != @intFromEnum(errors.Error.OK)) {
        testing.fail(1);
        return;
    }

    // Step 8: leg-B refresh — idc_read does not mutate VMAR state, so
    ...
    const cap_post_b = caps.readCap(cap_table_base, vmar_handle);
