...
//   MMIO VMAR whose caps.x bit is set together with a port_io
//   device_region in [2]. Per §[port_io_virtualization], port-IO
//   virtualization is implemented by trapping every CPU access to
//   the VMAR's range and decoding the faulting MOV — an executable
...
//   First, §[create_vmar] test 11 already rejects caps.mmio = 1 and
//   caps.x = 1 simultaneously with E_INVAL: the test child cannot
//   build an MMIO-flagged VMAR with the spec-violating caps.x bit at
//   all, since create_vmar refuses the construction up front. The
//   eventual faithful test must therefore obtain such a VMAR from a
...
//
//   With both the caps.x-on-mmio VMAR and the port_io device_region
...
//   plain MMIO VMAR (caps.x cleared so create_vmar accepts the
...
//     - caps = {r, w, mmio} so the VMAR is mmio-flagged. caps.x is
//       cleared because §[create_vmar] test 11 rejects caps.mmio = 1
//       with caps.x = 1; the eventual faithful test must obtain a
//       VMAR with caps.x set through some other path.
...
//         VMAR per §[var], cur_rwx ⊆ caps.{r, w}.
//     - pages = 1 — minimum-size MMIO VMAR.
...
//      MMIO VMAR sitting in `map = 0`. caps.x is intentionally
...
//   child without both a VMAR carrying caps.mmio + caps.x and a
...
//      able to deliver an MMIO-flagged VMAR handle whose caps.x bit
//      is set. Today the only minting path is `create_vmar`, which
//      §[create_vmar] test 11 closes; a faithful test needs either a
...
//     vx = forwarded mmio VMAR with caps.x = 1
...
pub fn main(cap_table_base: u64) void {
    ...
    // §[create_vmar] test 11 rejects caps.mmio = 1 with caps.x = 1;
    // the spec violation §[port_io_virtualization] test 03 turns on
    // (caps.x set on an mmio VMAR) cannot be constructed via
    // create_vmar, so the smoke prelude minimally builds the
    // caps.x-cleared mmio VMAR the eventual faithful test will reuse
    ...
    // forwards both a caps.x-set mmio VMAR and a port_io
    // device_region.
