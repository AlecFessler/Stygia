...
pub fn main(cap_table_base: u64) void {
    ...
    // VMAR can transition to map = 1 ahead of the remap call.
    ...
    // Step 2: regular VMAR in `map = 0`. caps.dma = 0 keeps test 05
    ...
    // a subset of VMAR.caps and pf.caps so tests 03/04 don't fire.
    const valid_rwx: u64 = 0b011;
    const rwx_with_reserved: u64 = valid_rwx | (@as(u64, 1) << 63);
    const result = syscall.remap(vmar_handle, rwx_with_reserved);
