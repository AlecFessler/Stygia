...
//
// Strategy
//   `idc_write` mutates VMAR contents but never touches the VMAR's `map`,
...
//     at offset 0 fits within the VMAR's 4 KiB size, offset is 8-byte
//     aligned, the handle has the `w` cap, count is in 1..125, no
//     reserved bits are set, and [1] is a valid VMAR — so none of
...
//     kernel-side VMAR field1 metadata is unchanged, so a refresh per
...
//     sentinel })` against the same VMAR — none of §[idc_write] tests
...
//      fire, and the call cannot mutate kernel-side VMAR field1
...
//      the VMAR metadata left the slot out of sync with the kernel's
...
pub fn main(cap_table_base: u64) void {
    ...
    // §[var] line 877 it starts at `map = 0`. The VMAR has the `w` cap
    ...
    // Step 3: idc_write against the fresh VMAR. count = 1, offset = 0
    // (8-byte aligned, fits within the VMAR's 4 KiB size); the [1]
    // handle is a valid VMAR with the `w` cap and no reserved bits set
    ...
    // kernel-side VMAR field1 metadata (idc_write only stores bytes into
    // the backing page), so the authoritative state remains exactly
    // what step 2 snapshotted.
    _ = syscall.idcWrite(vmar_handle, 0, &.{SENTINEL});

    // Step 4: leg-A refresh — the kernel's authoritative VMAR field1
    ...
    // the success branch.
    const ok_call = syscall.idcWrite(vmar_handle, 0, &.{SENTINEL});
    if (ok_call.v1 != @intFromEnum(errors.Error.OK)) {
        testing.fail(1);
        return;
    }

    // Step 8: leg-B refresh — idc_write does not mutate VMAR field1
