// Spec §[create_vmar] — test 18.
//
// "[test 18] on success, the caller receives a VMAR handle with caps
// = `[1].caps`."
//
// Strategy
//   Drive create_vmar down its success path with the same prelude as
//   create_var_05/runner/serial.zig — the root domain's self-handle
//   carries `crvr`, the vmar_inner_ceiling permits {r, w}, max_sz = 0
...
//
// Assertions
//   1: createVmar returned an error word in vreg 1 (success path failed).
...
pub fn main(cap_table_base: u64) void {
    const requested = caps.VmarCap{ .r = true, .w = true };
