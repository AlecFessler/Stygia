...
//  written into the VMAR starting at [2] offset."
//
// Strategy
...
//      so CPU loads through VMAR.base[..] observe the same physical
...
//      After this call, idc_write writes to the VMAR commit to the
//      page_frame and CPU loads through VMAR.base see them.
//   4. idcWrite(vmar_handle, /*offset=*/0, /*qwords=*/&.{ SENTINEL_LO,
...
//      VMAR.base + offset in offset order — vreg 3 -> offset 0,
//      vreg 4 -> offset 8.
//   5. Read VMAR.base[0] and VMAR.base[1] back through a volatile u64
...
//   3: VMAR.base[0] did not equal the sentinel passed in vreg 3.
//   4: VMAR.base[8] did not equal the sentinel passed in vreg 4.
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 2: regular VMAR with caps={r, w} and cur_rwx = r|w. The `w`
    ...
    // VMAR.base[0..4096] become (r|w) ∩ (r|w) = r|w; idc_write commits
    // to the page_frame and CPU loads through VMAR.base see them.
    ...
    // lines 1163 and 1170, the kernel pauses every EC in the VMAR's
    // owning domain for the duration of the call (a no-op here — the
    // caller is the only EC), reads count qwords from vregs 3..2+count,
    // and writes them into VMAR.base + offset in offset order
    // (vreg 3 -> offset 0, vreg 4 -> offset 8).
    const got = syscall.idcWrite(vmar_handle, 0, &.{ SENTINEL_LO, SENTINEL_HI });
    ...
    // Step 5c: VMAR.base[0] must equal the qword passed in vreg 3.
    if (qword_ptr[0] != SENTINEL_LO) {
        testing.fail(3);
        return;
    }

    // Step 5d: VMAR.base[8] must equal the qword passed in vreg 4.
