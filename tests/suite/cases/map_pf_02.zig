...
//   VMAR handle so map_pf clears the §[map_pf] test 01 path, and we
...
//     - test 04 (N == 0) — N = 1.
//     - test 05 (offset misaligned to VMAR sz) — offset = 0 is aligned
//       to any sz, including the VMAR's sz = 0 (4 KiB).
...
//   plus the freshly-minted VMAR slot. Slot 4095 — the maximum 12-bit
...
//      VMAR handle in vreg 1 (assertion 1 guards this precondition).
//   2. mapPf(vmar_handle, &.{ 0, 4095 }) — offset 0 is aligned, but
...
//   2: createVmar returned an error code in vreg 1 — the success-path
...
pub fn main(cap_table_base: u64) void {
    ...
    const result = syscall.mapPf(vmar_handle, &.{ 0, empty_slot });
