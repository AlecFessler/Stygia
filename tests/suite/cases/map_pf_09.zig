...
//   the spec text scopes the overlap on the *VMAR's* installed range,
...
//     - test 01 (VMAR invalid)  — same fresh VMAR handle from setup.
//     - test 02 (pf invalid)   — same fresh page_frame handle.
//     - test 03 (caps.mmio)    — VMAR built with mmio = 0.
//     - test 04 (N == 0)       — N = 1.
//     - test 05 (alignment)    — offset 0 is aligned to any sz.
//     - test 06 (pf sz < VMAR)  — both sz = 0.
//     - test 07 (range > VMAR)  — single 4 KiB pair fits a 1-page VMAR.
...
//      a regular VMAR with map = 0.
//   3. mapPf(vmar_handle, &.{ 0, pf_handle }) — must succeed
//      (precondition: an installation now exists at offset 0).
//   4. mapPf(vmar_handle, &.{ 0, pf_handle }) — must return E_INVAL
//      because offset 0 already has a mapping.
//
// Assertions
//   1: a setup syscall (createPageFrame, createVmar, or the first
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: stage a 4 KiB page_frame. sz = 0 matches the VMAR's
    // sz so §[map_pf] test 06 (pf sz smaller than VMAR sz) cannot
    ...
    // Step 2: build a regular (non-mmio, non-dma) 1-page VMAR with
    ...
    // VMAR's map is 1 with one installed page_frame covering offset
    // 0..4096. Any non-success result here means the precondition
    // for test 09 is broken — fail under assertion id 1.
    const first = syscall.mapPf(vmar_handle, &.{ 0, pf_handle });
    ...
    // (test 09 itself).
    const second = syscall.mapPf(vmar_handle, &.{ 0, pf_handle });
