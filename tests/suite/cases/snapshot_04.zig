...
//
// Strategy
//   §[snapshot] requires the source VMAR ([2]) to carry
//   `caps.restart_policy = 2` (preserve). Per §[var] VmarCap layout
...
//     - test 01 (target [1] not a valid VMAR) — pass a freshly-minted
//       VMAR handle.
//     - test 02 (source [2] not a valid VMAR) — same.
//     - test 03 (target.caps.restart_policy != 3) — mint the target
//       with `restart_policy = 3` (snapshot). Per §[create_capability_domain]
//       test 07's notes, the runner's `restart_policy_ceiling.vmar_restart_max`
...
//   default VmarCap values (`restart_policy = 0`, i.e. `free`), which
//   is unambiguously not 2.
//
// Action
//   1. createVmar(caps={r, w, restart_policy=3}, props=0b011, pages=1)
//      → target VMAR with `restart_policy = snapshot`.
//   2. createVmar(caps={r, w}, props=0b011, pages=1)
//      → source VMAR with default `restart_policy = 0` (free).
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: target VMAR with caps.restart_policy = 3 (snapshot). The
    // VmarCap layout puts restart_policy at bits 9-10. r|w gives the
    // VMAR the same effective surface as the source (sizes and shape
    ...
    // Step 2: source VMAR with default restart_policy = 0 (free) —
