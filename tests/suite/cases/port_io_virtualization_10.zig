...
//
// "[test 10] a `LOCK`-prefixed MOV targeting the VMAR delivers a
...
//      the VMAR's virtual range from the test child. Unlike the
...
//      installed a port_io device_region into the VMAR's range.
//      Without a port_io VMAR in scope the LOCK-MOV would fault
...
//   call site is wired up; the LOCK-prefixed-MOV-on-VMAR
...
//   LOCK-prefixed MOV targeting a successfully-mapped port_io VMAR
...
//
// Assertion
//   No assertion is checked — the LOCK-prefixed-MOV-on-VMAR
...
//   prelude itself (createVmar) is also reported as
...
//      against the mapped VMAR's virtual range. Inline assembly of
...
pub fn main(cap_table_base: u64) void {
    ...
    // decoder this test targets. The LOCK-prefixed-MOV-on-VMAR
    ...
    // LOCK-prefixed-MOV-on-VMAR protection_fault leg is unreachable
