// Spec §[capability_domain] acquire_vmars — test 03.
//
...
//
//   The libz `syscall.acquireVmars` wrapper takes `target: u12`, which
//   cannot carry reserved bits in [1]. We bypass that wrapper and
//   dispatch through `syscall.issueReg` directly so we can stuff bit
//   12 into vreg 1.
//
// Action
//   1. acquire_vmars(SLOT_SELF_IDC | (1 << 12)) — must return E_INVAL
//      (reserved bit 12 of [1] set; low 12 bits hold the valid id)
//
// Assertions
//   1: acquire_vmars with reserved bit 12 of [1] returned something
//      other than E_INVAL
...
pub fn main(cap_table_base: u64) void {
    ...
    // `syscall.acquireVmars` takes u12 and would truncate the reserved
    // bit before it reaches the kernel.
    const handle_with_reserved: u64 =
        @as(u64, caps.SLOT_SELF_IDC) | (@as(u64, 1) << 12);
    const result = syscall.issueReg(.acquire_vmars, 0, .{
