...
//   Test 11 fires only on a VMAR whose `map` field is 3 (demand). Per
...
//
//     create_vmar(caps={r, w}, props={sz=0, cur_rwx=0b011}, pages=2,
//                ...)                     -> regular VMAR, map = 0
//     <fault at VMAR.field0 + 0>           -> kernel demand-faults a
//                                            page; VMAR.map -> 3,
//                                            page installed at 0
//     <fault at VMAR.field0 + 0x1000>      -> second demand-fault;
//                                            map stays 3, page
//                                            installed at 0x1000
...
//      regular VMAR in `map = 0`, large enough for two 4-KiB
//      installations at offsets 0 and 0x1000.
//   3. mapPf(vmar_handle, &.{ 0, pf_a, 0x1000, pf_b }) — drives
//      `map` to 1 with two installations. Required so the following
//      `unmap` calls are not rejected by §[unmap] test 02
//      (`map = 0` => E_INVAL).
//   4. unmap(vmar_handle, &.{ pf_a }) — succeeds on the `map = 1`
...
//   5. unmap(vmar_handle, &.{ pf_b }) — succeeds on the `map = 1`
...
//   that issues a load at `VMAR.base + offset` from a controlled
//   trampoline, returns into the test EC with no register clobber,
//   and leaves `VMAR.map = 3` with one demand-allocated page per
//   faulted offset. The action then becomes:
//     create_vmar(..., pages=2)         -> regular VMAR, map = 0
//     <fault at VMAR.base + 0>          -> kernel demand-faults a
//                                         page, VMAR.map -> 3
//     <fault at VMAR.base + 0x1000>     -> second demand-fault, map
//                                         stays 3
...
pub fn main(cap_table_base: u64) void {
    ...
    _ = syscall.mapPf(vmar_handle, &.{ 0, pf_a, 0x1000, pf_b });
    ...
    // unreachable.
    _ = syscall.unmap(vmar_handle, &.{pf_a});
    ...
    _ = syscall.unmap(vmar_handle, &.{pf_b});
