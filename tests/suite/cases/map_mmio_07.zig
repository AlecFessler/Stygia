...
//     - a valid MMIO VMAR in [1] (caps.mmio = 1, caps.x = 0,
//       caps.dma = 0, props.sz = 0, props.cch = 1; per §[var]
//       create_vmar tests 08, 11, 13).
...
//   without a device_region: a freshly-minted MMIO VMAR has
//   `field1.map = 0` (per §[var] line 877) and, by §[var] field-
//   layout (line 911), its `field1.device` is 0 because the VMAR was
//   not produced by `create_vmar` with a bound device_region (DMA
//   path) and no successful `map_mmio` has run. The smoke confirms
//   that the *prelude shape* used by the eventual faithful test
//   succeeds: create_vmar with mmio caps lands a valid handle whose
//   field1 starts in the spec'd zero state. The post-map field1
//   transition itself cannot be checked here.
//
// Strategy (smoke prelude)
//   To stage the same MMIO VMAR a faithful test 07 would build, all
//   create_vmar precondition rules for caps.mmio = 1 must be
//   satisfied:
//     - caps.x = 0 (create_vmar test 11)
//     - caps.dma = 0 (create_vmar test 13)
//     - props.sz = 0 (create_vmar test 08, mmio requires 4 KiB pages)
...
//      MMIO VMAR with field1.map = 0 and field1.device = 0.
//
// Assertion
...
//   failure of create_vmar itself is also reported as pass-with-id-0
...
//     size matches a 4 KiB MMIO VMAR and forward it to the test child
...
//        side-effect of any subsequent VMAR-taking syscall, or via
...
//   re-deriving the MMIO-VMAR construction.
...
pub fn main(cap_table_base: u64) void {
    ...
    // Stage the same MMIO VMAR map_mmio_02 stages — caps = {r, w,
    // mmio}, props.sz = 0, props.cch = 1 (uc), cur_rwx = r|w. Per
    // §[var] line 877 the VMAR lands in field1.map = 0; per §[var]
