...
//   as a VMAR handle, so feeding it as [2] exercises the §[snapshot]
//   test 02 BADCAP gate on the source argument.
//
//   To isolate that gate from the §[snapshot] test 01 BADCAP gate on
//   [1], we must create [1] as a valid VMAR handle. Per
//   §[restart_semantics], the calling domain's
//   `restart_policy_ceiling.vmar_restart_max` bounds `caps.restart_policy`
//   on `create_vmar`. The runner's primary.zig grants the child
//   `vmar_restart_max = 3` (full snapshot policy), so a VMAR may be
...
//      must return a VMAR handle in vreg 1 (assertion 2 guards this
//      precondition).
//   2. snapshot(valid_var_handle, empty_slot=4095) — must return
//      E_BADCAP because [2] is not a valid VMAR handle.
//
// Assertions
//   1: snapshot returned something other than E_BADCAP.
//   2: createVmar failed — the success-path precondition is broken so
