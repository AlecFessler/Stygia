...
//                                       VMAR's `map` becomes 1.
//     - test 03 (map == 2 with N > 0) — VMAR caps.mmio = 0, so the
...
//   the VMAR (so `map = 1` and the not-installed gate is reachable),
//   and call `unmap(var, &.{ pf_b })`. pf_b is a valid page_frame
//   handle, but it was never installed in this VMAR, so the kernel
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: pf_a — the page_frame we'll install in the VMAR so
    ...
    // we will never install in the VMAR. This is the selector that
    ...
    // Step 3: regular 1-page VMAR (caps.mmio = 0, caps.dma = 0).
    ...
    // Step 4: install pf_a at offset 0. After this the VMAR's `map`
    // is 1, so §[unmap] test 02 (map == 0) can no longer fire.
    const install = syscall.mapPf(vmar_handle, &.{ 0, pf_a });
    ...
    // VMAR — §[unmap] test 05 demands E_NOENT.
    const result = syscall.unmap(vmar_handle, &.{pf_b});
