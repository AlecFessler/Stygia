...
//
// "[test 06] returns E_PERM if `vmar_outer_ceiling` is not a subset of
//  the caller's `vmar_outer_ceiling`."
//
// Strategy
//   `vmar_outer_ceiling` is an 8-bit subfield (bits 8-15) of
//   `ceilings_outer` (field1) per §[capability_domain]. The subset check
//   is bitwise: any bit set in [3].vmar_outer_ceiling that is clear in
//   the caller's `vmar_outer_ceiling` must surface E_PERM.
//
...
//   value and set it in [3].vmar_outer_ceiling while leaving every other
...
//
// Degraded smoke
//   The runner's primary domain currently sets `vmar_outer_ceiling` to
...
//   `vmar_outer_ceiling`, this test will start exercising the real rule
//   without code changes.
//
// Action
//   1. read caller self-cap field1 → caller_vmar_outer (bits 8-15)
//   2. if caller_vmar_outer == 0xFF, smoke-pass
//   3. else create_page_frame(caps={r,w}, props=0, pages=1) — must succeed
//   4. call create_capability_domain with [3].vmar_outer_ceiling
//      = caller_vmar_outer | (one bit clear in caller) — must return E_PERM
//
// Assertions
...
pub fn main(cap_table_base: u64) void {
    ...
    const caller_vmar_outer: u8 = @truncate((self_cap.field1 >> 8) & 0xFF);
    ...
    if (caller_vmar_outer == 0xFF) {
        testing.pass();
        return;
    }

    // Find the lowest-numbered clear bit in caller_vmar_outer and set it
    ...
    const new_vmar_outer: u8 = caller_vmar_outer | extra_bit;
    ...
    // only ceiling violation is vmar_outer_ceiling in [3]. The exact
    ...
    // and rewrite vmar_outer_ceiling with the superset value.
    //   bits  0-7   ec_outer_ceiling          = 0xFF
    //   bits  8-15  vmar_outer_ceiling         = caller | extra_bit
