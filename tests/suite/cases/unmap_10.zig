...
//   `map_pf` calls the VMAR has two installed page_frames at offsets 0
...
//
//   Two-page VMAR + two distinct page_frames so the first `unmap` (which
//   removes only pf_a) leaves pf_b installed and the VMAR's `map` must
...
//
// Assertions
//   1: setup failed — createPageFrame, createVmar, or the seeding mapPf
//      returned an error.
...
pub fn main(cap_table_base: u64) void {
    ...
    // Regular VMAR (caps.mmio = 0, caps.dma = 0). Two pages so the two
    ...
    // VMAR in `map = 1` with two installed page_frames.
    const r_seed = syscall.mapPf(vmar_handle, &.{ 0, pf_a, 0x1000, pf_b });
    ...
    const r_first = syscall.unmap(vmar_handle, &.{pf_a});
    ...
    const r_second = syscall.unmap(vmar_handle, &.{pf_b});
