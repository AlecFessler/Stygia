// Spec §[create_vmar] — test 17.
//
// "[test 17] returns E_INVAL if any reserved bits are set in [1] or [2]."
//
// Strategy
//   §[create_vmar] pins the layout of [1] and [2] explicitly:
...
//
//   To isolate the reserved-bit check we make every other create_vmar
...
//   field.
//
//   The libz `syscall.createVmar` wrapper takes u64 args, so it does
...
//
// Action
//   1. createVmar with caps = (valid_caps | (1<<63)), valid props
//      — must return E_INVAL.
//   2. createVmar with valid caps, props = (valid_props | (1<<63))
//      — must return E_INVAL.
//
// Assertions
...
pub fn main(cap_table_base: u64) void {
    ...
    // Same valid-create_vmar setup as create_var_05: caps={r,w},
    ...
    const valid_caps: u64 = @as(u64, vmar_caps.toU16());
    ...
    const r1 = syscall.issueReg(.create_vmar, 0, .{
    ...
    const r2 = syscall.issueReg(.create_vmar, 0, .{
        .v1 = valid_caps,
