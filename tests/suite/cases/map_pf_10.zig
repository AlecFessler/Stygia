...
//     primary's serial VMAR cannot succeed inside a test child,
//     because the child's table holds self / initial_ec / self_idc /
//     port and nothing else.
//
//   map = 3 (demand): per §[var] (line 877) a regular VMAR transitions
...
//     a VMAR's range is to dereference a pointer at `VMAR.base + 0`,
...
//     executing at whatever its prior context was, but the VMAR is
//     now permanently in `map = 3` *for the remaining test*. Even
//     if the fault completes cleanly, there is no syscall in the v3
//     surface that drives a VMAR into `map = 3` *without* an actual
//     CPU page fault, and there is no spec'd faulting-helper that a
//     test can call to trigger demand-paging on a VMAR whose base is
...
//   VMAR (caps.mmio = 0, caps.dma = 0) created without explicit
//   mapping starts in `map = 0` per §[var], a single map_pf into it
//   succeeds (transitioning `map` to 1 per §[map_pf] test 11), and a
//   second non-overlapping map_pf into the same VMAR also succeeds
...
//       offsets are 0 and 0x1000, the VMAR is 2 pages of 4 KiB, and
...
//   3. mapPf(vmar_handle, &.{ 0, pf1 }) — must succeed (transitions
//      map 0 -> 1 per §[map_pf] test 11).
//   4. mapPf(vmar_handle, &.{ 0x1000, pf2 }) — must also succeed
...
//   device_region whose size matches a 4 KiB MMIO VMAR and forward it
...
//   an MMIO VMAR. So the `map = 2` arm of test 10 is structurally
//   unreachable as written: an MMIO VMAR is rejected by test 03
...
//
//   For the `map = 3` arm — the test must drive the VMAR into demand
//   mode. The cleanest path is a runner-side helper that issues a
//   read at `VMAR.base` from a controlled trampoline, returns into
//   the test EC with no register clobber on the test's side, then
//   calls map_pf on the now-`map = 3` VMAR. The action becomes:
//     create_vmar(...) -> regular VMAR, map = 0
//     <load from VMAR.base>            -> kernel demand-faults a page,
//                                        VMAR.map -> 3
...
pub fn main(cap_table_base: u64) void {
    ...
    // map_pf calls below both fit inside the VMAR's range.
    ...
    _ = syscall.mapPf(vmar_handle, &.{ 0, pf1 });
    ...
    _ = syscall.mapPf(vmar_handle, &.{ 0x1000, pf2 });
