...
//
// Strategy
//   The spec requires that whenever map_mmio sees a valid VMAR in [1],
...
//   (because the call errored before touching the VMAR), then a
//   refresh must leave field1 bit-for-bit unchanged.
//
//   We mint an MMIO VMAR using the same prelude as map_mmio_02 and
//   map_pf_03: caps = {r, w, mmio}, props = {sz=0, cch=1 (uc),
//   cur_rwx=0b011}, pages=1, preferred_base=0, device_region=0. This
//   succeeds and yields a VMAR handle with `caps.mmio = 1` and
//   `field1.map = 0`.
//
...
//   [1] gate passes because the VMAR is valid, so the spec's refresh
...
//      handleType was not virtual_memory_address_region — precondition
...
pub fn main(cap_table_base: u64) void {
    // Build a valid MMIO VMAR. Same construction as map_mmio_02 /
    // map_pf_03 — caps.mmio = 1 forces props.sz = 0 (create_vmar test
    ...
    // because mmio_var_handle is a valid VMAR — that's the
    ...
    // fired first), so the authoritative VMAR state is unchanged. A
