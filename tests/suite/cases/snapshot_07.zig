...
//   field0/field1 layout exposes the VMAR's own state (base, page_count,
...
//   vmar_inner_ceiling permitting r/w and vmar_restart_max = 3
...
//   bits must be zero; both must be valid VMAR handles. The minimum
//   caps in caps.r/w (we set r and w on each) keep §[create_vmar]
//   tests 02-13 from rejecting either VMAR.
//
// Action
//   1. createVmar(target,   caps={r,w,restart_policy=3}, props=0b011, 1 page) — var1.
//   2. createVmar(source A, caps={r,w,restart_policy=2}, props=0b011, 1 page) — var2.
//   3. createVmar(source B, caps={r,w,restart_policy=2}, props=0b011, 1 page) — var3.
//   4. snapshot(var1, var2) — must return OK (binds A as the source).
//   5. snapshot(var1, var3) — must return OK (replaces A with B).
//
// Assertions
//   1: any of the three createVmar calls returned an error code
...
fn createTargetVar() ?caps.HandleId {
    const c = caps.VmarCap{
        .r = true,
        .w = true,
        .restart_policy = 3, // snapshot — required for §[snapshot] [1]
    };
    const r = syscall.createVmar(@as(u64, c.toU16()), PROPS, PAGES, 0, 0);
