...
//   VMAR via `map_mmio` is only legal when the VMAR's cache type is
...
//   Build an MMIO-flagged VMAR whose `cch` is *not* uc so the prelude
...
//       the VMAR to match that device_region's port_count (rounded
...
//
// Action
//   1. createVmar(caps={r, w, mmio}, props={sz = 0, cch = 0,
//                cur_rwx = 0b011}, pages = 1, preferred_base = 0,
//                device_region = 0) — must succeed; gives a valid
//      mmio-flagged VMAR with cch = 0 (wb) sitting in `map = 0`.
...
//     create_vmar(caps={r, w, mmio}, props={sz = 0, cch = 0 (wb),
...
pub fn main(cap_table_base: u64) void {
    ...
    // Build an MMIO-flagged VMAR with cch = 0 (wb). caps.mmio = 1
    // forces props.sz = 0 (§[create_vmar] test 08); cch = 0 is the
    // spec violation the faithful §[port_io_virtualization] test 02
    // turns on; pages = 1 is the minimum-size MMIO VMAR.
