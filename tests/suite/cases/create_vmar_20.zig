// Spec §[create_vmar] — test 20.
//
// "[test 20] on success, field1 contains `[2].props` together with
//  `[3]` pages."
//
// Strategy
//   Mint a regular VMAR (mmio=0, dma=0) along the same prelude every
//   other "success-path" create_vmar test uses (see create_var_05 and
...
//
//   For a fresh non-DMA VMAR with no `map_pf` / `map_mmio` yet, both
...
//      — must return a VMAR handle in vreg 1.
//   2. readCap(cap_table_base, returned_handle_id) — read field1.
//   3. Compare field1 against the expected packed encoding.
//
// Assertions
//   1: createVmar returned an error code in vreg 1 (success path
//      precondition broken — we cannot proceed to verify field1).
//   2: the returned slot's handleType is not virtual_memory_address_region
//      (kernel installed something other than a VMAR — also kills the
//      precondition).
...
pub fn main(cap_table_base: u64) void {
    ...
    // non-DMA VMAR with no installations: map = 0, device = 0.
