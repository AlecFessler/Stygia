...
//   so the only spec-mandated failure path on `acquire_vmars` is the
...
//
//   With the cap stripped, `acquire_vmars(self_idc)` must report
...
//   2. acquire_vmars(SLOT_SELF_IDC)                    — must return E_PERM.
//
// Assertions
//   1: restrict returned a non-OK error word.
//   2: acquire_vmars returned something other than E_PERM.
...
pub fn main(cap_table_base: u64) void {
    ...
    // acquire_vmars is the cap-presence one.
