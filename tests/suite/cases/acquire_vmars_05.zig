// Spec §[capability_domain] acquire_vmars — test 05.
//
...
//   VMAR set the test fully controls: its own. acquire_vmars on this
...
//
//   The exact baseline VMAR count for the freshly-spawned test domain
...
//
//     1. Call acquire_vmars(self_idc) — record N0 (baseline).
//     2. Create K = 2 new VARs, each backed by a page frame and
//        map_pf'd at offset 0 to drive `map` from 0 to 1. Per §[var]
//        a VMAR with `map = 1` is included in acquire_vmars's reply.
//     3. Call acquire_vmars(self_idc) — record N1.
//     4. Assert N1 == N0 + K.
//
...
//   the VMAR's base address, which would mutate the test EC's data
...
//   §[acquire_vmars] (and §[acquire_ecs] uses the same encoding). The
//   libz `syscall.acquireVmars` wrapper returns a `RecvReturn` with
//   the post-syscall word in `.word`, so we shift-and-mask out the
//   count.
//
// Action
//   1. acquire_vmars(SLOT_SELF_IDC)                 — record baseline N0
//   2. for i in 0..K:
//        create_page_frame(1 page, r|w)
//        create_vmar(1 page, caps.r|w, props.cur_rwx=r|w)
//        map_pf(var, offset=0, pf)
//   3. acquire_vmars(SLOT_SELF_IDC)                 — record N1
//   4. assert N1 == N0 + K
//
// Assertions
//   1: baseline acquire_vmars returned an error in vreg 1
//   2: a setup syscall (create_page_frame / create_vmar / map_pf)
//      returned an error word
//   3: second acquire_vmars returned an error in vreg 1
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: baseline.
    const baseline = syscall.acquireVmars(caps.SLOT_SELF_IDC);
    ...
    // §[create_vmar] props: cur_rwx in bits 0-2; sz=0 (4 KiB) default;
    // cch=0 (wb) default. Set r|w to match VMAR caps.
    ...
    while (i < K) {
        ...
        const cvar = syscall.createVmar(var_caps_word, props_rw, 1, 0, 0);
        ...
        const mp = syscall.mapPf(vmar_handle, &.{ 0, @as(u64, pf_handle) });
    ...
    const after = syscall.acquireVmars(caps.SLOT_SELF_IDC);
