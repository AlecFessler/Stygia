...
//     - a valid VMAR handle whose caps carry mmio = 1, so test 01
//       (E_BADCAP for an invalid VMAR) does not fire either.
//
...
//     - The root domain's vmar_inner_ceiling permits mmio (the same
//       construction is used by runner/serial.zig).
//   With caps = {r, w, mmio} and props = {sz=0, cch=1 (uc),
//   cur_rwx=0b011} the create_vmar prelude succeeds and we get a VMAR
...
//
//   With both prerequisites in hand we issue map_pf on the MMIO VMAR
...
//      succeed.
//   2. createVmar(caps={r,w,mmio}, props={sz=0, cch=1 (uc),
...
//
// Assertions
//   1: vreg 1 was not E_PERM after mapPf on an MMIO VMAR.
...
pub fn main(cap_table_base: u64) void {
    ...
    // Build an MMIO-capable VMAR (no map_mmio needed — test 03 turns
