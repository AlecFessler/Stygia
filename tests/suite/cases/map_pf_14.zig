...
//
// Strategy
//   `map_pf` mutates the kernel-side VMAR state (`map` and, on DMA,
...
//     Build a regular 2-page VMAR with `caps = {r, w}`, `cur_rwx =
...
//     none of §[map_pf] tests 01-10 fire (VMAR/pf valid, mmio = 0,
//     N = 1, offset 0 aligned, pf.sz == VMAR.sz, pair fits, no second
...
//
//   Leg B (error refresh)
//     Issue a second `map_pf(var, &.{ 0, pf })` against the same VMAR.
...
//      success refresh leg).
//   4. readCap(cap_table_base, vmar_handle) — field1's `map` (bits
//      39-40) must be 1.
//   5. mapPf(var, &.{ 0, pf }) — must return E_INVAL (overlap with
//      step 3's installation, §[map_pf] test 09).
//   6. readCap(cap_table_base, vmar_handle) — field1's `map` must
...
pub fn main(cap_table_base: u64) void {
    // Step 1: 2-page page_frame, sz = 0 so it matches the VMAR's sz
    // (test 06 inert) and a single pair at offset 0 covers the VMAR's
    ...
    // Step 2: regular 2-page VMAR (caps.mmio = 0, caps.dma = 0). Per
    ...
    // `sync` required.
    const cap_after_ok = caps.readCap(cap_table_base, vmar_handle);
    if (mapField(cap_after_ok.field1) != 1) {
        testing.fail(2);
        return;
    }

    // Step 5: identical pair against the same VMAR. §[map_pf] test 09
    ...
    const cap_after_err = caps.readCap(cap_table_base, vmar_handle);
