...
//
// Strategy
//   `remap` mutates the kernel-side VMAR state (`cur_rwx` on the
...
//   3. remap(vmar_handle, 0b001) — must return E_INVAL (§[remap] test
...
//      succeed.
//   6. mapPf(vmar_handle, &.{ 0, pf }) — must succeed (sets `map = 1`,
//      precondition for the success-leg refresh).
//   7. remap(vmar_handle, 0b001) — must succeed; per §[remap] test 07
//      `cur_rwx` becomes 0b001.
//   8. readCap(cap_table_base, vmar_handle) — field1's `cur_rwx` must
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 3: remap on a fresh VMAR. §[remap] test 02 fires (`map = 0`)
    // and the call returns E_INVAL before any kernel-side mutation.
    // The [1] gate has already passed because `vmar_handle` is a valid
    // VMAR — that's the precondition for the spec's "[1] valid →
    // field0/field1 refreshed" requirement.
    const err_call = syscall.remap(vmar_handle, NEW_CUR_RWX);
    ...
    const ok_call = syscall.remap(vmar_handle, NEW_CUR_RWX);
