// Spec §[reply] reply — test 12.
//
// "[test 12] returns E_BADCAP if any pair entry's source handle id is
...
//     - present a reply handle whose `xfer` cap is set (else test 09
//       fires first),
//     - carry 1 ≤ N ≤ 63 (else test 10 / bare-reply path),
//     - keep all reserved bits cleared in the syscall word (else test
//       02) and the pair entry (else test 11),
//     - keep pair-entry source ids unique (else test 16; trivial here
//       because N=1).
//
...
//   call reply with pair_count=1 and a single pair entry whose source
...
//   06 / reply test 11) passes; the unused reserved bits stay zero.
//
// reply ABI
//   Spec §[reply]:
//
//     reply([1] reply, [128-N..127] pair_entries when N > 0) -> void
//       syscall_num = 52
//       syscall word bits 12-19: pair_count (0..63)
//
//   With pair_count=1 the single pair entry occupies vreg 127. Per
//   §[syscall_abi] vreg N for 14 ≤ N ≤ 127 lives at [rsp + (N-13)*8]
//   when the syscall executes. For vreg 127 that is [rsp + 912]. libz's
//   `reply` wrapper covers only the bare-reply path, so this test
//   issues the syscall via inline asm: subq the high-vreg pad, write
//   the entry into the vreg-127 slot, push the syscall word so the
...
//   5. reply(reply_handle_id, pair_count=1,
...
//   5: reply returned something other than E_BADCAP
...
// Inline-asm reply with pair_count=1. The single pair entry is placed at
// vreg 127. On x86 vreg 127 = [rsp + 912]; on aarch64 vreg 127 =
// [sp + 768]. Returns the kernel's vreg 1 (rax/x0) — the error code or
// 0 on success.
fn replyOneEntry(reply_handle_id: u12, pair_entry: u64) u64 {
    // Spec §[syscall_abi] / §[reply]: syscall_num in bits 0-11;
    // pair_count `N` in bits 12-19; reply_handle_id in bits 20-31. N=1.
    const word: u64 = (@as(u64, @intFromEnum(syscall.SyscallNum.reply)) & 0xFFF) |
        ((@as(u64, 1) & 0xFF) << 12) |
...
pub fn main(cap_table_base: u64) void {
    ...
    // kernel mints at recv has its `xfer` cap (else reply test 09 fires
    // before test 12's source-id check).
    ...
    // Step 5: reply with one pair entry whose source id is 4095 — far
    // outside the test EC's used handle range. Per §[reply] test 12,
    // this must surface E_BADCAP.
    //
    ...
    const result = replyOneEntry(reply_handle_id, pair_entry);
