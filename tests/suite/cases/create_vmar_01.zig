...
//   `crvr = true`. To force the create_vmar E_PERM path we must drop
...
//   3: create_vmar returned something other than E_PERM
...
pub fn main(cap_table_base: u64) void {
    ...
    // create_vmar args: caps with r/w set, props.sz=0, pages=1, base=0, dev=0.
    // VmarCap r=bit2, w=bit3.
    const vmar_caps: u64 = (1 << 2) | (1 << 3);
    const props: u64 = 0;
    const pages: u64 = 1;
    const result = syscall.createVmar(vmar_caps, props, pages, 0, 0);
