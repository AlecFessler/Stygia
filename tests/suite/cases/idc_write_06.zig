...
//
// Strategy
//   §[idc_write] takes a VMAR handle in [1], a byte offset in [2], and
...
//     - test 02 (no `w` cap)            — VMAR is created with w = true.
...
//                                          observably asserts.
//
//   The libz `syscall.idcWrite` wrapper takes `vmar_handle: u12` which
...
//
// Action
//   1. createVmar(caps={w}, props={cur_rwx=w, sz=0, cch=0}, pages=1)
//      — must succeed, gives a regular VMAR with the `w` cap so the
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: regular VMAR with `w` cap so the test 02 (E_PERM) gate
    ...
    // the VMAR's size, keeping tests 03 and 05 inert. v3 holds the
