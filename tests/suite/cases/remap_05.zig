...
//  has bit 2 (x) set."
//
// To reach this gate the VMAR under test must be created with
// caps.dma = 1. Per §[create_vmar] tests 14/15 a dma create_vmar
...
// create_vmar prelude — and therefore §[remap] test 05 — structurally
...
//   1. createVmar(caps={r,w,dma}, props={cur_rwx=0b011, sz=0, cch=0},
//                pages=1, preferred_base=0, device_region=found).
//      Per §[create_vmar] tests 11/12 caps.x must be 0 for an mmio
//      or dma VMAR, so the new VMAR's caps.x = 0 — but caps.dma = 1.
//   2. remap(var, new_cur_rwx = 0b100). Bit 2 (x) is set, and the
//      VMAR has caps.dma = 1, so §[remap] test 05 demands E_INVAL.
//      (Earlier remap gates are inert: test 01 needs an invalid
//      handle; test 02 needs map=0 or map=2 — but after a fresh
//      create_vmar with caps.dma=1, `map` is 0, which collides with
//      test 02. To isolate test 05 the VMAR must be in `map = 1` or
//      `map = 3` first, which requires installing or demand-paging
//      a page_frame into a dma VMAR. That extra setup is left for
...
//   `remap(var, 0b100)` on a dma VMAR (the spec assertion under
//   test).
...
pub fn main(cap_table_base: u64) void {
    _ = findDmaDevice(cap_table_base) orelse {
        ...
        // child's cap table. §[remap] test 05 requires a VMAR with
        // caps.dma = 1, which in turn requires a dma device_region
        // for create_vmar; the v3 runner does not forward one to
    ...
    // page_frame into a dma VMAR to escape §[remap] test 02
...
fn findDmaDevice(cap_table_base: u64) ?caps.HandleId {
    ...
    while (slot < caps.HANDLE_TABLE_MAX) {
        ...
        if (c.handleType() == .device_region) {
            ...
            // §[create_vmar] test 15, so filter for it here.
