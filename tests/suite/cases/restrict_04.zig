...
//
// "[test 04] returns E_PERM if the handle is a VMAR handle and [2].caps'
//  restart_policy (bits 9-10) numeric value exceeds the handle's current
//  restart_policy."
//
// Strategy
//   restart_policy on a VMAR handle is a 2-bit numeric enum in cap bits
//   9-10 (0=free, 1=decommit, 2=preserve, 3=snapshot) that uses
//   numeric monotonicity, not bitwise subset semantics. Test 04 is the
//   VMAR-handle analogue of test 03 (which covers the EC-handle path).
//
//   Mint a regular VMAR (mmio=0, dma=0) with `restart_policy = 0`
...
//   with E_PERM.
//
//   Choices that keep create_vmar off the other E_INVAL/E_PERM paths:
//     - caps.r = caps.w = true, no x/mmio/dma — within
//       vmar_inner_ceiling = 0x01FF granted by the runner.
...
//
// Action
//   1. create_vmar(caps={r,w,rp=0}, props={cur_rwx=r|w}, pages=1, ...)
//   2. restrict(var, caps={r,w,rp=1})
//
// Assertions
//   1: setup syscall failed (create_vmar returned an error word)
...
pub fn main(cap_table_base: u64) void {
    ...
    const expanded = caps.VmarCap{
        .r = true,
        .w = true,
        .restart_policy = 1,
    };
    const new_caps_word: u64 = @as(u64, expanded.toU16());
    const result = syscall.restrict(vmar_handle, new_caps_word);
