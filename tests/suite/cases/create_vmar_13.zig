// Spec §[create_vmar] create_vmar — test 13.
//
// "[test 13] returns E_INVAL if caps.mmio = 1 and caps.dma = 1."
//
// Strategy
//   `mmio` and `dma` are mutually exclusive on a VMAR's caps. An MMIO
//   VMAR is bound to a port_io / mmio device_region by `map_mmio`; a
//   DMA VMAR routes the bound device's IOMMU accesses to page_frames.
//   A single VMAR cannot serve both roles, so the kernel must reject
//   any `create_vmar` whose [1] caps word has both bits set.
//
//   The runner (runner/primary.zig) installs the test domain with a
//   permissive `vmar_inner_ceiling`, so r/w and mmio are permitted by
...
//
// Action
//   create_vmar(caps={r,w,mmio,dma}, props={cur_rwx=0b011, sz=0, cch=0},
