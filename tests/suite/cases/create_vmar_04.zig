...
//   The mock runner in `tests/suite/runner/primary.zig` mints child
//   capability domains with `vmar_inner_ceiling = 0x01FF`, which
//   includes mmio (bit 5 within the 16-bit vmar_inner_ceiling, i.e. u64
...
//   `vmar_inner_ceiling` lacks mmio. Test 04's guard therefore cannot
...
//
// Strategy
//   Mirror the mmio createVmar shape from runner/serial.zig (cch=1 uc,
//   pages=1, vmar_caps with r|w|mmio) but flip props.sz from 0 to 1 so
...
//   accepts. Other guards are dodged the same way the other create_vmar
//   tests dodge them:
