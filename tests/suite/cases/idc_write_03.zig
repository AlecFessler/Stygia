...
//     - [1] must be a valid VMAR handle with the `w` cap (defeats
//       tests 01 and 02),
//     - count must be in [1, 125] (defeats test 04),
//     - offset + count*8 must fit within the VMAR's size (defeats
//       test 05),
//     - reserved bits in [1] and [2] must be zero (defeats test 06).
//
//   Setup:
//     1. createPageFrame(caps={r,w}, props=0, pages=1) — provides
//        backing storage so the VMAR is non-empty.
//     2. createVmar(caps={r,w}, props=0, pages=1) — VMAR.caps has `w`
//        (defeats test 02), sz=0 (4 KiB), pages=1 → size = 4096 bytes.
//     3. mapPf(var, &.{ 0, pf }) — install the page_frame so the VMAR
...
//     VMAR, so test 05 cannot preempt. No reserved bits are set in
//     either vreg.
//
// Assertions
//   1: vreg 1 was not E_INVAL after the idc_write call (the spec
//      assertion under test).
//   2: a setup syscall returned an error (createPageFrame, createVmar,
...
pub fn main(cap_table_base: u64) void {
    ...
    // VMAR so the only E_INVAL the kernel can surface in this test is
    ...
    // Step 2: VMAR with caps = r|w (the `w` cap defeats §[idc_write]
    // test 02). props = 0 → cur_rwx = 0, sz = 0, cch = 0. pages = 1
    // gives a 4 KiB VMAR, large enough that offset = 1, count = 1
    ...
    // Step 3: install the page_frame at offset 0 so the VMAR has live
    ...
    const result = syscall.idcWrite(vmar_handle, 1, &.{1});
