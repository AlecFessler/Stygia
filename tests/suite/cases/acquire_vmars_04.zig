// Spec §[capabilities] acquire_vmars — test 04.
//
...
//               which sets aqvr; this IDC is a valid `acquire_vmars`
...
//   domain with at least one `map=1` or `map=3` VMAR, and (b) the
...
//   IS this domain. We mint exactly one map=1 VMAR by:
//     1. create_page_frame with caps.r|w
//     2. create_vmar with caps.r|w and props.cur_rwx = r|w (map = 0)
//     3. map_pf installing the page_frame at offset 0 (transitions
//        VMAR.field1.map from 0 to 1 per §[map_pf] test 11)
//   That guarantees `acquire_vmars` would return at least one handle.
//
...
//   With zero free slots and at least one map=1 VMAR in the target,
//   `acquire_vmars` cannot accommodate any returned handle and must
//   surface E_FULL.
//
// Action
//   1. create_page_frame(caps={r,w})           — must succeed
//   2. create_vmar(caps={r,w}, props.cur_rwx=rw) — must succeed
...
//   5. acquire_vmars(SLOT_SELF_IDC)             — must return E_FULL
//
// Assertions
//   1: setup syscall create_page_frame returned an error
//   2: setup syscall create_vmar returned an error
...
//   5: acquire_vmars returned something other than E_FULL
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: page frame to back the VMAR.
    ...
    // Step 2: VMAR with cur_rwx = r|w. caps.r and caps.w are required
    // for cur_rwx.r|w to satisfy §[create_vmar] test 16.
    ...
    // Step 3: install the page frame; this transitions VMAR.field1.map
    // from 0 to 1, so the VMAR is now in the `acquire_vmars`-eligible
    // set per §[acquire_vmars] test 05.
    ...
    // Step 5: with zero free slots and at least one eligible VMAR in the
    // target domain (this domain), the kernel cannot mint the returned
    // handle and must surface E_FULL.
    const result = syscall.acquireVmars(caps.SLOT_SELF_IDC);
