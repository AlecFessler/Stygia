// Spec §[create_vmar] create_vmar — test 10.
//
...
//       cannot fire.
//     - test 02 (E_PERM, r/w/x not subset of vmar_inner_ceiling.r/w/x):
//       the runner sets vmar_inner_ceiling = 0x01FF (all VMAR-cap bits
//       set, including r, w, x), so {r,w} is trivially a subset.
//     - test 03 (E_PERM, caps.max_sz exceeds vmar_inner_ceiling.max_sz):
//       runner's vmar_inner_ceiling.max_sz = 3 (all bits set); caps
