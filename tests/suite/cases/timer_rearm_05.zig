...
// kernel-side propagation walk plus user-mode mode switch; under
// load (full 481-test suite at smp=4) that gap has been observed
// to span tens of ms (raced 50 ms ~1 in 5 runs). 500 ms gives ~10×
// headroom. Spec §[timer_rearm] pins the reset semantics, not the
// absolute period.
