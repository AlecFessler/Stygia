// Spec §[create_vmar] create_vmar — test 02.
//
// "[test 02] returns E_PERM if caps' r/w/x bits are not a subset of the
//  caller's `vmar_inner_ceiling`'s r/w/x bits."
//
// DEGRADED SMOKE VARIANT
//   The strict E_PERM path here requires the caller's
//   `vmar_inner_ceiling` (self-handle field0 bits 8-23) to have at least
...
//   as a child capability domain with `vmar_inner_ceiling = 0x01FF`
...
//   the runner-supplied `vmar_inner_ceiling` (so the test child is born
//   with one of r/w/x cleared), or (b) extend the test to spawn a
//   nested sub-domain with reduced ceilings and observe its
//   `create_vmar` attempt fail. Neither path exists in the v0 runner;
//   `restrict` cannot lower ceilings (it only tightens caps).
//
//   This smoke variant instead asserts the complementary positive
//   observation: a `create_vmar` whose caps r/w/x bits are *exactly* the
//   subset of the caller's `vmar_inner_ceiling` (read from slot 0)
...
//
// Strategy
//   1. Read self-handle field0 from slot 0; extract vmar_inner_ceiling
//      bits 8-23 and mask out the r/w/x bits (VmarCap bits 2..4).
//   2. Build a `VmarCap` whose r/w/x bits exactly mirror the ceiling's
//      r/w/x bits — no other VMAR-specific bits set so unrelated tests
...
//      props.cch = 0.
//   5. Call create_vmar. The exact-subset r/w/x must not surface E_PERM.
//
// GAP
...
//   `vmar_inner_ceiling`, this file lives in the manifest as a smoke
...
//   vmar_inner_ceiling). The current pass uses assertion id 0 — see the
...
pub fn main(cap_table_base: u64) void {
    // §[capability_domain] self-handle field0 layout: vmar_inner_ceiling
    ...
    // VmarCap bits 2..4 are r/w/x. Extract just those bits from the
    // ceiling so the request is by construction a subset.
    const rwx_mask: u64 = 0b1_1100; // bits 2..4
    const ceiling_rwx: u64 = vmar_inner_ceiling & rwx_mask;

    // Compose a caps word with only the ceiling's r/w/x bits set. Other
    // VmarCap bits (move/copy, mmio, max_sz, dma, restart_policy) stay
