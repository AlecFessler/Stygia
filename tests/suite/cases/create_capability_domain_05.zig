...
//
// "[test 05] returns E_PERM if `vmar_inner_ceiling` is not a subset of
//  the caller's `vmar_inner_ceiling`."
//
// Strategy
//   The faithful test mints a capability domain whose `ceilings_inner`
//   sets a bit in bits 8-23 (vmar_inner_ceiling) that the caller does
//   not have set in its own `vmar_inner_ceiling`. The kernel must
...
//   vmar_inner_ceiling (bits 8-23) is 0x01FF — every defined bit set
...
//   Read the caller's actual `vmar_inner_ceiling` from slot 0 of the
...
//
// Action
//   1. read self-handle field0 (slot 0) → caller_vmar_inner_ceiling
//   2. compose ceilings_inner with bits 8-23 = caller_vmar_inner_ceiling
...
// Runner-supplied baseline for the non-vmar_inner_ceiling fields.
// Mirrors runner/primary.zig's ceilings_inner; we substitute bits 8-23
// with the caller's actual vmar_inner_ceiling read from slot 0.
const RUNNER_CEILINGS_INNER_NON_VMAR: u64 = 0x001C_011F_3F00_00FF;
const RUNNER_CEILINGS_OUTER: u64 = 0x0000_003F_03FE_FFFF;

pub fn main(cap_table_base: u64) void {
    const self_cap = caps.readCap(cap_table_base, caps.SLOT_SELF);
    const caller_vmar_inner_ceiling: u64 = (self_cap.field0 >> 8) & 0xFFFF;

    const ceilings_inner: u64 =
        RUNNER_CEILINGS_INNER_NON_VMAR | (caller_vmar_inner_ceiling << 8);
    ...
    // attributable to the vmar_inner_ceiling subset rule, which cannot
