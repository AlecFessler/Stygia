...
//   The full assertion enumerates every (VMAR.cur_rwx, pf.rwx) pair —
//   nine non-trivial intersections (excluding pf.x, which §[create_vmar]
//   tests 11/12 already gate on the VMAR side and which would also
//   require an executable VMAR for the CPU to actually fetch from a
...
//     - per-test cur_rwx settings on the VMAR;
...
//   path probe: a VMAR with caps={r,w} and cur_rwx=r|w, mapped with a
//   page_frame whose caps={r,w}. The intersection (r|w) ∩ (r|w) = r|w,
//   so a CPU write followed by a CPU read at VMAR.base + 0 must both
...
//      with the VMAR's cur_rwx = r|w to yield effective r|w.
//   2. create_vmar(caps={r,w}, props={cur_rwx=r|w, sz=0, cch=0},
//                 pages=1, preferred_base=0, device_region=0) — must
//      succeed; gives a regular VMAR (caps.dma=0, caps.mmio=0) in
//      `map = 0` whose base is kernel-chosen and reported in field0.
//   3. map_pf(var, &.{ 0, pf }) — installs the page_frame at offset
//      0 of the VMAR, transitioning `map` to 1 per §[map_pf] test 11.
//      Effective permissions on VMAR.base[0..4096] become r|w per
//      §[map_pf] test 12 (the rule under test).
//   4. Write a known sentinel byte at VMAR.base[0]; read it back.
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 1: page_frame with caps={r, w}. No x — the VMAR is also
    ...
    // Step 2: regular VMAR with caps={r, w} and cur_rwx = r|w. By
    // §[var] line 877 the VMAR starts in `map = 0` because no explicit
    ...
    const var_base: u64 = cvar.v2; // §[create_vmar] test 19: field0 = base.

    // Step 3: install the page_frame at offset 0. Per §[map_pf] test
    // 11, `map` transitions 0 -> 1. Per the rule under test, effective
    // permissions on VMAR.base[0..4096] become (cur_rwx) ∩ (pf.caps r/w/x)
    // = (r|w) ∩ (r|w) = r|w.
    ...
    // Step 4: write + read at VMAR.base[0]. Both accesses use the
