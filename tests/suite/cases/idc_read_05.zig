...
//   VMAR's byte size (`page_count * sz`). Earlier §[idc_read] gates must
...
pub fn main(cap_table_base: u64) void {
    ...
    // Step 2: regular VMAR, exactly one 4 KiB page. caps.r is required
    // for the §[idc_read] perm gate (test 02); caps.w lets the same
    // VMAR receive a writable page_frame mapping in step 3 without
    ...
    // of map, but a backed VMAR is the ordinary debugger-read shape.
    ...
    // VMAR's 0x1000-byte size by 8, which §[idc_read] test 05 demands
    // surfaces as E_INVAL.
    const result = syscall.idcRead(vmar_handle, 0xFF8, 2);
