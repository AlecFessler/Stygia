...
//   0..2 of vreg 2) against the VMAR handle's own r/w/x cap bits. To
//   isolate it from the other E_INVAL paths in remap, we drive the
//   VMAR into a state where every other precondition is satisfied:
...
//        when we request r|w|x on the VMAR.
//     2. createVmar(caps={r,w}, props=0b011, pages=1) — VMAR.caps has
...
//     VMAR.caps.x is 0, so the kernel must surface E_INVAL per
...
pub fn main(cap_table_base: u64) void {
    ...
    // left to fail is the VMAR-caps subset check from test 03.
    ...
    // Step 2: VMAR with caps = r|w only (no x), caps.dma = 0,
    ...
    // Step 4: remap with new_cur_rwx = 0b111 (r|w|x). VMAR.caps.x = 0,
    // so the requested bits are not a subset of [1]'s caps r/w/x —
    // §[remap] test 03 must surface E_INVAL.
    const result = syscall.remap(vmar_handle, 0b111);
