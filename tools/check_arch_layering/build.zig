...
// installs to `zig-out/bin/check_arch_layering` so existing callers
...
pub fn build(b: *std.Build) void {
    ...
    const exe_path = link.addOutputFileArg("check_arch_layering");
    link.addFileArg(obj.getEmittedBin());
    link.addArg("-lsqlite3");

    const install = b.addInstallBinFile(exe_path, "check_arch_layering");
    b.getInstallStep().dependOn(&install.step);

    const run_cmd = std.Build.Step.Run.create(b, "run check_arch_layering");
    ...
    const run_step = b.step("run", "Run the arch-layering analyzer");
