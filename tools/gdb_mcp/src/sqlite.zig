//! Minimal SQLite C-API wrapper, copy of tools/callgraph_mcp/src/sqlite.zig.
//! Read-only access to the per-(arch, commit_sha) callgraph DB built by
//! tools/indexer. Surfaces only what the gdb_mcp resolvers need.
