...
// installs to `zig-out/bin/check_gen_lock` so existing callers
...
pub fn build(b: *std.Build) void {
    ...
    const exe_path = link.addOutputFileArg("check_gen_lock");
    link.addFileArg(obj.getEmittedBin());
    link.addArg("-lsqlite3");

    const install = b.addInstallBinFile(exe_path, "check_gen_lock");
    b.getInstallStep().dependOn(&install.step);

    const run_cmd = std.Build.Step.Run.create(b, "run check_gen_lock");
